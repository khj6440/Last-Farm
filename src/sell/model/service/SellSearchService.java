	...
	public SellCategoryPage searchBoxSorting(int reqPage, String sortingTab, String searchWord) {
		...
		if (pageNo != 1) {
			pageNavi += "<a class='btn btn-outline-secondary' href='/sellSearchNationalFrm?reqPage="
					+ (pageNo - pageNaviSize) +"&sortingTab="+sortingTab+"&searchWord="+searchWord+"'>Before</a>";
		}
		for (int i = 0; i < pageNaviSize; i++) {
			if (reqPage == pageNo) {
				pageNavi += "<span class='btn btn-outline-secondary'>" + pageNo + "</span>";
			} else {
				pageNavi += "<a class='btn btn-outline-secondary' href='sellSearchNationalFrm?reqPage=" + pageNo + "&sortingTab="+sortingTab+"&searchWord="+searchWord+"'>"
						+ pageNo + "</a>";
			}
			pageNo++;
			if (pageNo > totalPage) {
		...
		if (pageNo < totalPage) {
			pageNavi += "<a class='btn btn-outline-secondary'" // 다음 버튼 만들기
					+ "href='/sellSearchNationalFrm?reqPage=" + pageNo + "&sortingTab="+sortingTab+"&searchWord="+searchWord+"'>After</a>";
		}
	...
	public SellCategoryPage selectList(int reqPage, String sortingTab) {
		...
		for (int i = 0; i < pageNaviSize; i++) {
			if (reqPage == pageNo) {
				...
				pageNavi += "<a class='btn btn-outline-secondary' href='sellSearchNationalFrm?reqPage=" + pageNo +"&sortingTab="+sortingTab+"'>"
						+ pageNo + "</a>";
