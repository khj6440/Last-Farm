...
#addrSearchBtn, #addPhoto, #addMPhoto {
	background-color: white;
	border: 1px solid gray;
	height: 30px;
	width: 120px;
...
#addrSearchBtn:hover, #addPhoto:hover, #addMPhoto:hover {
	background-color: #ffac05;
	color: #4a2100;
	cursor: pointer;
}
...
<script>
	...
	//지역 주소 추가&삭제 버튼
	$(function() {
		$(".chooseNation").prop("checked", true);
		$(".chooseRtab").hide();
	...
	//배송비 선택 기능
	$(function() {
		$(".dfradio").eq(0).prop("checked", true);
	...
	$(function() {
		$("#addMPhoto").click(function() {
			$("input[multiple='multiple']").click();
		});
	});
</script>
<body>
		...
		<table>
			...
			<tr>
				...
				<td><input type="file" name="thumbnail" id="thumbnail"
					class="fileattach" hidden>
					<button type="button" id="addPhoto" class="btn btn-primary">사진
						첨부</button></td>
			</tr>
			<tr>
				<th>물품 사진 등록</th>
				<td><input multiple="multiple" type="file" name="imgFilepath[]"
					class="fileattach" hidden>
					<button type="button" id="addMPhoto" class="btn btn-primary">
						다중 사진 첨부</button></td>
